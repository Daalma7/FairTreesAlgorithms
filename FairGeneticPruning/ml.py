...
PATH_TO_RESULTS = os.path.dirname(os.path.dirname(os.path.dirname(__file__))) + '/results/FGP'
...
def test_and_save_results(x_test, y_test, prot_test, classifiers, gen_stats_df, population_df, nind, ngen, dat, var, seed, obj, extra):
    ...
    save_population_name = f"{PATH_TO_RESULTS}/population/{dat}/{dat}_seed_{seed}_var_{var}_gen_{ngen}_indiv_{nind}_model_FGP_obj_{obj_str}{extra_str}.csv"
    save_gen_name = f"{PATH_TO_RESULTS}/generation_stats/{dat}/{dat}_seed_{seed}_var_{var}_gen_{ngen}_indiv_{nind}_model_FGP_obj_{obj_str}{extra_str}.csv"
    save_pareto_run_name = f"{PATH_TO_RESULTS}/pareto_individuals/runs/{dat}/{dat}_seed_{seed}_var_{var}_gen_{ngen}_indiv_{nind}_model_FGP_obj_{obj_str}{extra_str}.csv"
...
def correct_pareto_optimal(dat, var, objectives, nind, ngen, seed, i, extra, struc):
    ...
    all_indivs = []
    pareto_optimal = []
    pareto_fronts = pd.read_csv(f"{PATH_TO_RESULTS}/pareto_individuals/runs/{dat}/{dat}_seed_{seed + i}_var_{var}_gen_{ngen}_indiv_{nind}_model_FGP_obj_{obj_str}{extra_str}.csv")
    hyperparameters = []
...
def calculate_pareto_optimal(dat, var, objectives, nind, ngen, seed, runs, extra, struc, correct=True):
    ...
    pareto_optimal_df.to_csv(f"{PATH_TO_RESULTS}/pareto_individuals/overall/{dat}/{dat}_seed_{seed}_var_{var}_gen_{ngen}_indiv_{nind}_model_FGP_obj_{obj_str}{extra_str}.csv", index=False)
